...
(async()=>{
 ...
 let lastInjection,activate,update,current={id:1,url:'http://127.0.0.1:8765/mock'};
 w.chrome={storage:{session:{get:async()=>({connection:{port:8765,key:'x'.repeat(40)}}),set:async()=>{},remove:async()=>{}}},tabs:{query:async()=>[current],onActivated:{addListener:f=>activate=f},onUpdated:{addListener:f=>update=f}},scripting:{executeScript:async p=>{lastInjection=p;return p.files?[]:[{result:{ok:true,result:p.args[1]==='fill'?{message:'1 CRN filled and verified.'}:{mapping:[{id:'crn-1',crn:'10119'}],already:false}}}];}}};
 ...
 current={id:3,url:'https://suis.sabanciuniv.edu/prod/bwskfreg.P_Regs'};activate({tabId:3});
 w.document.getElementById('preview').click();await tick();assert(!w.document.getElementById('fill').disabled);assert.equal(lastInjection.args[0],'suis');assert.equal(lastInjection.args[1],'preview');
 w.document.getElementById('fill').click();await tick();assert.equal(lastInjection.args[0],'suis');assert.equal(lastInjection.args[1],'fill');assert.match(w.document.getElementById('status').textContent,/filled and verified/);assert(w.document.getElementById('fill').disabled);
