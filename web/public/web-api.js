...
window.SniperWeb=(()=>{
 ...
 let phone={enabled:false,details:false,last:'Telegram notifications are off.'};try{phone={...phone,...JSON.parse(sessionStorage.getItem(TKEY)||'{}')};}catch{}
 ...
 async function phoneApi(action,b){if(action==='connect'){if(!/^\d{5,}:[A-Za-z0-9_-]{20,}$/.test(b.token))throw Error('Invalid bot token format.');phone={token:b.token,enabled:false,details:false,code:'RS-'+Array.from(crypto.getRandomValues(new Uint8Array(16)),x=>x.toString(16).padStart(2,'0')).join(''),expires:Date.now()+600000,last:'Send the pairing command to your new bot, not BotFather.'};await telegram('getMe');return phoneSave();}if(action==='pair'){if(!phone.code||Date.now()>phone.expires)throw Error('Pairing expired. Save the bot token again.');const u=await telegram('getUpdates',{timeout:0,limit:100,allowed_updates:['message']});const hits=[...new Set(u.filter(x=>x.message?.chat?.type==='private'&&x.message.text==='/start '+phone.code).map(x=>x.message.chat.id))];if(hits.length!==1)throw Error('Pairing command not found in one private chat. Send it to your bot, then confirm.');phone.chat=hits[0];delete phone.code;phone.last='Telegram connected. Enable notifications below.';}else if(action==='config'){if(!phone.chat)throw Error('Connect Telegram first.');phone.enabled=!!b.enabled;phone.details=!!b.details;phone.last=phone.enabled?'Telegram notifications enabled while this tab is running.':'Telegram notifications off.';}else if(action==='test'){if(!phone.chat)throw Error('Connect Telegram first.');await telegram('sendMessage',{chat_id:phone.chat,text:'Registration Sniper: Telegram notification test.'});phone.last='Test sent.';}else if(action==='disconnect'){phone={enabled:false,details:false,last:'Disconnected.'};sessionStorage.removeItem(TKEY);return phoneState();}return phoneSave();}
 async function api(path,b){
  ...
  if(path==='seats/config'){if(s.running)throw Error('Pause monitoring before editing the watch list.');if(!/^\d{6}$/.test(b.term)||!Array.isArray(b.crns)||b.crns.length>80||b.crns.some(x=>!/^\d{5}$/.test(x))||![30,60,120,300,600].includes(b.interval))throw Error('Use up to 80 valid CRNs and a supported interval.');const old=s;s={...s,...b,crns:[...new Set(b.crns)],reason:''};try{syncFollow(getPlan());if(old.term!==s.term){s.observations={};s.events=[];}persistWatch();revision++;return snapshot();}catch(e){s=old;throw e;}}
