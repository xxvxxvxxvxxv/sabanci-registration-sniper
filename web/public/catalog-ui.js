...
function isActive(r){return r.selected;}
...
function renderCatalog(){
 ...
 const rows=catalogData.courses.filter(c=>(!plannedOnly||plan.courses.some(r=>r.selected&&(r.catalog_base===c.code||key(r.course)===key(c.code))))&&(!raw||key(c.code).includes(q)||(c.name+' '+c.offerings.map(o=>o.instructor+' '+o.crn).join(' ')).toLowerCase().includes(raw)));
 $('catalog-list').innerHTML=rows.map(c=>{const active=plan.courses.filter(r=>isActive(r)&&r.catalog_base===c.code).length;return `<article class="catalog-course"><button class="catalog-course-title" data-course="${esc(c.code)}" aria-expanded="${expandedCourse===c.code}"><span><b>${esc(c.code)}</b><small>${esc(c.name)}</small>${active&&window.registrationBadge?window.registrationBadge({course:c.code}):''}</span><span class="badge">${active?active+' selected':c.offerings.length+' sections'}</span></button>${expandedCourse===c.code?`<div class="component-list">${[...new Set(c.offerings.map(o=>o.type))].map(t=>`<h3>${esc(typeName(t))}</h3>${c.offerings.filter(o=>o.type===t).map(o=>{const chosen=selectionFor(c.code,t),on=chosen?.crns===o.crn;return `<button class="section-option ${on?'chosen':''}" data-crn="${esc(o.crn)}" ${mismatch||busy?'disabled':''} aria-pressed="${on}"><span class="section-option-head"><b>${esc(o.section)} <span>${on?'✓':''}</span></b><code>${esc(o.crn)}</code></span><span>${esc(o.instructor)}</span><small>${esc(offerText(o)).replaceAll('\n','<br>')}</small></button><a class="section-link" href="${courseURL(plan.term,o.crn)}" target="_blank" rel="noreferrer">Course page ↗</a>`;}).join('')}`).join('')}</div>`:''}</article>`;}).join('')||'<p class="panel-intro">No matching courses.</p>';
 renderTimetable();
}
...
function renderTimetable(){
 ...
 if(window.renderRegistrationSelection)window.renderRegistrationSelection(active);
...
async function chooseSection(crn){
 ...
 if(current?.crns===crn){existing.selected=false;}else{
  ...
  if(existing){const fresh=catalogRow(c,o);if(existing.meetings!==fresh.meetings||existing.section!==fresh.section){existing.meetings=fresh.meetings;existing.section=fresh.section;existing.verified=false;}existing.selected=true;}
