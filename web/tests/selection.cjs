...
(async()=>{
 ...
 try{
  ...
  for(const id of ['watch-follow','watch-backups','watch-plan'])assert.equal($(id),null);
  ...
  assert.equal($('timetable-scale').value,'comfortable','Remember the previous view choice');
  assert.equal(w.document.querySelectorAll('.timetable-detail').length,0);
  assert.equal(w.getComputedStyle(w.document.querySelector('.tt-room')).display,'block');
  assert.equal(w.getComputedStyle(w.document.querySelector('.tt-block')).overflow,'hidden');
  const fullHeight=parseFloat(w.document.querySelector('.tt-day').style.height);
  $('timetable-scale').value='fit';$('timetable-scale').onchange();
  assert(parseFloat(w.document.querySelector('.tt-day').style.height)<fullHeight);
  assert.equal(w.getComputedStyle(w.document.querySelector('.tt-room')).display,'none');
