...
(async()=>{
 ...
 w.fetch=async(url,opts={})=>{requests.push([url,opts]);let data;if(url.startsWith('/api/catalog'))data=cat;else if(url.startsWith('/api/seats?'))data={observations:Object.fromEntries(new URL(url,'https://sniper.test').searchParams.get('crns').split(',').map(id=>[id,{capacity:20,actual:20-seatCount,remaining:seatCount,available:seatCount,checked_at:new Date(w.Date.now()).toISOString()}])),pending:[]};else throw Error('Unexpected network destination '+url);return {ok:true,json:async()=>JSON.parse(JSON.stringify(data))};};
 ...
 assert.equal(w.document.querySelectorAll('#view-extension img').length,0);assert.equal(w.document.querySelectorAll('.extension-instructions li').length,3);
