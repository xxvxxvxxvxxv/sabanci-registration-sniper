(function(root){
 ...
 async function read(tabId,copyOnly=false){
  const tab=await chrome.tabs.get(tabId);
  if(!accepts(tab.url))throw Error('Open the Registration Sniper website tab and connect again.');
  const results=await chrome.scripting.executeScript({target:{tabId},world:'MAIN',func:async(expectedOrigin,copyOnly=false)=>{
   try{
    if(location.origin!==expectedOrigin||!window.SniperWeb)throw Error('Website is loading or this page is unsupported.');
    const p=await window.SniperWeb.api('plan'),draft=copyOnly?{ready:true,crns:[...new Set(p.courses.filter(r=>r.selected).flatMap(r=>String(r.crns||'').trim().split(/\s+/).filter(Boolean)))],errors:[],warnings:[]}:await window.SniperWeb.api('draft',p),current=await window.SniperWeb.api('plan');
